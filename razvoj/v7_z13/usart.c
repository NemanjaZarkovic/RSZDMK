...
{
	int8_t c;
	int32_t op1 = 0, op2 = 0, res = 0;
	int8_t sgn = 1;
    int8_t operation;

	if (usartCheck() == '-')		// Ako je pre cifre bio minus, podesavamo znak
	{
		usartGetChar();
		sgn = -1;
	}
	...
	{
		c = usartGetChar();
		op1 = op1 * 10 + (c - '0');						// Postojeci mnozi sa 10 i dodaje vrednost nove cifre
	}
	...
	{
		c = usartGetChar();
		op2 = op2 * 10 + (c - '0');						// Postojeci mnozi sa 10 i dodaje vrednost nove cifre
	}
